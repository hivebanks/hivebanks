...
//判断验证码发送数量是否超过最大限制
    $phone_code_num_limit = ba_phone_code_limit_check($phone);

    if ($phone_code_num_limit > 40)
        exit_error('108', 'no times for send code');
//获取最新一条发送记录
    $phone_code_last_time = get_ba_log_bind_by_variable('phone_code', $phone);
// 获取最新的创建记录
    $row = get_ba_id_by_variable("phone", $phone);
    //判断资金密码与账号密码是否相同
    if($get_pass_word_hash == $hash){
        exit_error('109','资金密码与账户密码不能相同');
    }
// 获取绑定信息日志表该用户最新的数据
    $rec = get_ba_log_bind_by_variable("phone_code", $phone);
    // 判断验证码是否为系统发送
