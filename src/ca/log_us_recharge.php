...
// 通过id获取用户基本信息
if ($type == '1') {
    $row = get_ca_recharge_request_ca_id($ba_id,"0");
}elseif ($type == '2') {
    $row = get_ca_recharge_request_ca_id($ba_id,"1");
}elseif ($type == '3'){
    $row = get_ca_recharge_request_ca_id($ba_id,"2");
}else {
    exit_error('131',"非法参数");
}
//获取充值列表基本信息
...
foreach ($row as $for_row) {
    $new_row["lgl_amount"] = floatval($for_row["lgl_amount"]);
    ...
    $new_row["tx_detail"] = json_decode(get_ca_asset_account_ca_id($for_row["ca_account_id"]));
    $new_rows[] = $new_row;
}
