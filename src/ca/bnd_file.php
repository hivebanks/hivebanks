...
//获取当前用户的绑定信息
$bind_info_us = get_ca_bind_info_by_token($ca_id);
foreach ($bind_info_us as $ca_info)
{
    if($ca_info['bind_name'] == $file_type){
        //更新该条数据
        $ret_us = upd_bind_info_for_us_id($ca_id,$file_type);
        if(!$ret_us)
...
if($ret_bind){
    exit_error('110','The binding already exists please try again！');
}
$ret = bind_log_info($ca_id, $data_bind);
if(!$ret){
    exit_error('105','Binding failed, please try again!');
}
//安全等级提升
