...
if($row['ba_id'] == null || $row['bind_flag'] == 9){
...
// 获取最新登录失败信息
$row_f = get_row_by_ba_id($row['ba_id']);
if($row_f){
  $count_error = $row_f['count_error'];
  $limit_time = $row_f['limt_time'];
  if($limit_time > $timestamp){
    $time_difference = $limit_time - $timestamp;
    exit_error('120',$time_difference);
    }
}
// 判断密码是否正确
$check_pass = check_pass($row['ba_id'],$pass_word_hash,'password_login');
if(!$check_pass){
 // 是否存在失败数据，并对失败数据进行记录
  if($row_f){
    $row_fail = $row_f;
    ...
    $ins_log_login_fail = creat_ba_log_login_fail($row_fail);
    exit_error('116', pow(2,$row_fail['count_error']));
  }else{
    $row_fail['ba_id'] =$row['ba_id'];
    $row_fail['ba_ip'] = get_int_ip(get_ip());
...
$timestamp += 2*60*60;
$des = new Des();
$encryption_code = $row['ba_id'] .',' . $timestamp . ',' . $salt;
