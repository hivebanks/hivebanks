...
//       insID            成功   
//======================================
function  ins_us_lgn_login($log_data)
{  
  $db = new DB_COM();
  $sql = $db->sqlInsert("us_log_login", $log_data);
  $q_id = $db->query($sql);
  if ($q_id == 0)
    return 0;
  return $db->insertID();
...
{
    $db = new DB_COM();
    $sql = "SELECT * FROM us_log_login WHERE us_id = '{$us_id}'";
    $db -> query($sql);
...
{
  $db = new DB_COM();
  $sql = "SELECT lgn_type,us_ip,ip_area,ctime FROM us_log_login ";
