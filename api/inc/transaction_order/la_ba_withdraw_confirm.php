...
function base_ba_withdraw_confirm($rows,$transfer_tx_hash){
    ...
    $new_lock_amount = $ba_row["lock_amount"] - $rows["base_amount"];
    if ($new_lock_amount < 0){
        $db->Rollback($pInTrans);
        exit_error("订单异常，金额不对,联系管理员");
    }

    $sql = "UPDATE ba_base SET lock_amount = '{$new_lock_amount}' WHERE ba_id = '{$rows["agent_id"]}'";
    $db->query($sql);
    $count = $db->affectedRows($sql);
    if (!$count){
        $db->Rollback($pInTrans);
