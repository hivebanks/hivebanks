...
function us_withdraw_quest($data) {
    ...
    $sql = "SELECT base_amount,lock_amount FROM ca_base WHERE ca_id = '{$data["agent_id"]}' limit 1";
    $db->query($sql);
    $rows = $db->fetchRow();
    ...
    $sql = "UPDATE ca_base SET base_amount = '{$new_base_amount}', lock_amount = '{$new_lock_amount}' WHERE ca_id = '{$data["agent_id"]}'";
    $db->query($sql);
