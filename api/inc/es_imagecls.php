...
class es_imagecls
{
	...
	/**
	 * $gen 0 等比缩放  1等比缩放 白色填充  2 裁剪 已最小为基准
 	 */
	public function thumb($image,$maxWidth=200,$maxHeight=50,$gen = 0,$interlace=true,$filepath = '',$is_preview = true)
    {
        ...
        if($info !== false)
		{
			...
			if($maxWidth > 0 && $maxHeight > 0)
			{
				...
				$img_ratio = $srcWidth/$srcHeight;
				
				if($gen==2)
				{
					if ($target_ratio > $img_ratio) {
						$width = $maxWidth;
						$height = $img_ratio * $maxHeight;
						$x = ($srcWidth - ($target_ratio * $height)) / 2;
						$y = 0;
					} else {
						$width = $maxWidth;
						$height = $maxWidth / $img_ratio;
						$x = 0;
						$y = ($srcHeight - ( (1 / $target_ratio) * $srcWidth)) / 2;
					}
				...
				if($gen==1){
					...
					$white  = imagecolorallocate($thumbImg, $red, $green, $blue);
					imagefilledrectangle($thumbImg, 0, 0, $maxWidth, $maxHeight, $white);
				}
				else{
					//创建缩略图
		            if($type!='gif' && function_exists('imagecreatetruecolor'))
					...
					$white  = imagecolorallocate($thumbImg, 0, 0, 0);
					imagefilledrectangle($thumbImg, 0, 0, $width, $height, $white);
				}
			}
            ...
            if($type!='gif' && function_exists('imagecreatetruecolor')){
            	...
            	$cropped_image = imagecreate($width, $height);
            }
            
            // 复制图片
            if(function_exists("imagecopyresampled")){
            	if($gen == 1)
            		imagecopyresampled($thumbImg, $srcImg, ($maxWidth-$width)/2, ($maxHeight-$height)/2, 0, 0, $width, $height, $srcWidth, $srcHeight);
            	elseif($gen == 2){
                ...
                imagecopyresized($thumbImg, $cropped_image, 0, 0, 0, 0, $maxWidth, $maxHeight,  $width,$height);
