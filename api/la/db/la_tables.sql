...
CREATE TABLE `us_invite` (
  ...
  `time_reg_action` bigint(20) DEFAULT NULL COMMENT '注册时间',
  `time_reg_success` bigint(20) DEFAULT NULL COMMENT '注册成功时间'
...
DROP TABLE IF EXISTS `la_log_login`;
CREATE TABLE `la_log_login` (
  `hash_id` char(64) CHARACTER SET ascii NOT NULL COMMENT 'HASH值',
  `prvs_hash` char(64) CHARACTER SET ascii NOT NULL DEFAULT '' COMMENT '上一HASH值',
  `ttl_id` char(36) CHARACTER SET ascii NOT NULL COMMENT '操作员ID',
  `lgn_type` char(10) CHARACTER SET ascii NOT NULL COMMENT '登录类型',
  `ttl_ip` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '操作员IP地址',
  `ip_area` varchar(255) DEFAULT NULL COMMENT 'IP地区',
  `utime` int(11) DEFAULT '0' COMMENT '更新时间戳',
  `ctime` datetime DEFAULT NULL COMMENT '提交时间',
  PRIMARY KEY (`hash_id`) USING BTREE
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='LA登录记录';

DROP TABLE IF EXISTS `la_log_login_fail`;
CREATE TABLE `la_log_login_fail` (
  `log_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '日志ID',
  `ttl_id` char(36) CHARACTER SET ascii NOT NULL COMMENT '用户ID',
  `ttl_ip` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '用户IP地址',
  `lgn_type` char(10) CHARACTER SET ascii NOT NULL COMMENT '登录类型',
  `count_error` tinyint(4) DEFAULT '0' COMMENT '登录错误次数',
  `limt_time` int(11) DEFAULT '0' COMMENT '限制时间戳',
  `ctime` datetime DEFAULT NULL COMMENT '错误时间',
  PRIMARY KEY (`log_id`) USING BTREE
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='LA登录失败记录';
